...
writer = SummaryWriter(log_dir='runs/har_experiment')
...
scheduler = OneCycleLR(optimizer, max_lr=Config.learning_rate, steps_per_epoch=len(train_loader), epochs=Config.epochs)
scaler = GradScaler()
...
def train():
    ...
    for epoch in range(Config.epochs):
        ...
        print(f"[Epoch {epoch+1}/{Config.epochs}] Train Loss: {epoch_loss:.4f} | Val Loss: {val_loss:.4f} | Acc: {acc:.4f} | F1: {f1:.4f}")
    ...
    save_checkpoint(model, optimizer, Config.epochs, val_loss, Config.checkpoint_path)
